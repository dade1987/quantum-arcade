...
export function twoWaves(host, opts = {}) {
  const cfg = Object.assign({
    ...
    onChange: null, title: 'Due onde, una somma', subtitle: 'cosa succede se le sposti?',
  }, opts);
  ...
  const y1 = t => st.A1 * Math.cos(2 * Math.PI * st.f * t + st.p1 * Math.PI / 180);
  const y2 = t => st.A2 * Math.cos(2 * Math.PI * st.f2 * t + st.p2 * Math.PI / 180);
  ...
  const waves = new Stage(w.body, {
    ...
    draw(ctx, s) {
      ...
      const curve = (fn, col, lw, alpha = 1, dash = null) => {
        ...
        for (let i = 0; i <= 320; i++) { const t = (i / 320) * 2; const X = px(t), Y = py(fn(t)); i ? ctx.lineTo(X, Y) : ctx.moveTo(X, Y); }
        ctx.stroke(); ctx.restore();
      };
      curve(y1, COL.cyan, 2, .9, [5, 4]);
      curve(y2, COL.amber, 2, .9, [5, 4]);
      curve(t => y1(t) + y2(t), COL.pink, 3.2, 1);

      text(ctx, 'onda A', rect.x + 6, rect.y + 10, { size: 11, color: COL.cyan });
      text(ctx, 'onda B', rect.x + 62, rect.y + 10, { size: 11, color: COL.amber });
      text(ctx, 'A + B', rect.x + 120, rect.y + 10, { size: 11, color: COL.pink });
      text(ctx, 'tempo →', rect.x + rect.w, rect.y + rect.h + 15, { size: 10, align: 'right', color: '#5b6b90' });
    },
  });
  waves.pause();
  ...
  const out = readout('');
  const sA1 = slider({ label: 'Ampiezza <b>A</b>', min: 0, max: 1.4, step: .01, value: st.A1, oninput: v => { st.A1 = v; upd(); } });
  const sP1 = slider({ label: 'Fase <b>A</b>', min: -180, max: 180, step: 1, value: st.p1, fmt: v => v + '°', oninput: v => { st.p1 = v; upd(); } });
  const sA2 = slider({ label: 'Ampiezza <b>B</b>', min: 0, max: 1.4, step: .01, value: st.A2, oninput: v => { st.A2 = v; upd(); } });
  const sP2 = slider({ label: 'Fase <b>B</b>', min: -180, max: 180, step: 1, value: st.p2, fmt: v => v + '°', oninput: v => { st.p2 = v; upd(); } });
  w.body.appendChild(controls(sA1.root, sP1.root, sA2.root, sP2.root));

  if (!cfg.lockFreq) {
    const sF1 = slider({ label: 'Frequenza <b>A</b>', min: .5, max: 4, step: .1, value: st.f, fmt: v => v.toFixed(1) + ' Hz', oninput: v => { st.f = v; upd(); } });
    const sF2 = slider({ label: 'Frequenza <b>B</b>', min: .5, max: 4, step: .1, value: st.f2, fmt: v => v.toFixed(1) + ' Hz', oninput: v => { st.f2 = v; upd(); } });
    w.body.appendChild(controls(sF1.root, sF2.root));
  }

  w.body.appendChild(h('div', { style: { marginTop: '10px' } }, buttons([
    { label: 'In fase (0°)', onclick: () => { sP1.value = 0; sP2.value = 0; st.p1 = 0; st.p2 = 0; upd(); } },
    { label: 'Opposte (180°)', onclick: () => { sP1.value = 0; sP2.value = 180; st.p1 = 0; st.p2 = 180; upd(); } },
    { label: 'A 90°', onclick: () => { sP1.value = 0; sP2.value = 90; st.p1 = 0; st.p2 = 90; upd(); } },
  ])));
  ...
  function upd() {
    ...
    out.set(`Somma delle due frecce: lunghezza <span class="p">${mag.toFixed(3)}</span>, angolo ${(((Math.atan2(sy, sx) * 180 / Math.PI) % 360 + 360) % 360).toFixed(0)}°\n` +
      `Massimo possibile: ${(st.A1 + st.A2).toFixed(2)} (in fase) · Minimo possibile: ${Math.abs(st.A1 - st.A2).toFixed(2)} (in opposizione)`);
...
export function manyArrows(host, opts = {}) {
  ...
  const stage = new Stage(w.body, {
    ...
    draw(ctx, s) {
      ...
      const mag = Math.hypot(sx, sy);
      text(ctx, `${st.n} frecce, ognuna lunga 1`, 12, 18, { size: 11.5, color: COL.txt });
      text(ctx, `somma = ${mag.toFixed(2)}  (massimo possibile: ${st.n})`, 12, 36, { size: 12, color: mag < st.n * 0.25 ? COL.red : COL.green });
      text(ctx, mag < st.n * 0.25 ? 'CANCELLAZIONE' : (mag > st.n * 0.9 ? 'ALLINEAMENTO PERFETTO' : 'somma parziale'), 12, 56, { size: 12, color: mag < st.n * 0.25 ? COL.red : COL.amber });
      if (cfg.onChange) cfg.onChange({ n: st.n, spread: st.spread, mag });
    },
  });
  const fx = attachFX(stage);   // scintille, lampi e suono ai traguardi
  stage.pause();
  const sN = slider({ label: 'Quante frecce', min: 2, max: 40, step: 1, value: st.n, fmt: v => v + '', oninput: v => { st.n = v; gen(); stage.redraw(); } });
  const sS = slider({ label: 'Disordine (0° = tutte uguali)', min: 0, max: 360, step: 5, value: st.spread, fmt: v => v + '°', oninput: v => { st.spread = v; gen(); stage.redraw(); } });
  w.body.appendChild(controls(sN.root, sS.root));
  w.body.appendChild(h('div', { style: { marginTop: '10px' } }, buttons([{ label: '🎲 rimescola', onclick: () => { st.seed++; gen(); stage.redraw(); } }])));
  w.setFoot('Metti il disordine a <b>0°</b>: la somma vale esattamente quanto il numero di frecce. Portalo a <b>360°</b>: la somma crolla vicino a zero, per quanto siano tante. Questo semplice fatto è il motore della trasformata di Fourier <b>e</b> di ogni algoritmo quantistico.');
  return { stage };
}
