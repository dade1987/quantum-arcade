...
export function polaroidLab(host, opts = {}) {
  ...
  const w = widget(host, { title: 'Fotoni e filtri polaroid', subtitle: 'l\'esperimento dei tre filtri' });
  ...
  const stage = new Stage(w.body, {
    ...
    draw(ctx, s) {
      ...
      filters.forEach((ang, i) => {
        ...
        text(ctx, `passa ${(f.steps[i] * 100).toFixed(0)}%`, fx, cy + 82, { size: 10, align: 'center', color: '#7f8fb3' });
      });
      drawBeam(x, s.w - 16, intensity, COL.green);
      text(ctx, 'sorgente', 20, cy - 44, { size: 11, color: '#ffe9a8' });
      const tot = f.total;
  ...
  const s1 = slider({ label: 'Filtro 1', min: 0, max: 180, step: 5, value: st.a1, fmt: v => v + '°', oninput: v => { st.a1 = v; upd(); } });
  const s2 = slider({ label: 'Filtro finale', min: 0, max: 180, step: 5, value: st.a2, fmt: v => v + '°', oninput: v => { st.a2 = v; upd(); } });
  const s3 = slider({ label: 'Filtro in mezzo', min: 0, max: 180, step: 5, value: st.a3, fmt: v => v + '°', oninput: v => { st.a3 = v; upd(); } });
  w.body.appendChild(controls(s1.root, s2.root, s3.root));
  w.body.appendChild(h('div', { style: { marginTop: '10px' } }, buttons([
    { label: '➕ metti/togli il filtro in mezzo', class: 'sm primary', onclick: () => { st.useThird = !st.useThird; upd(); } },
    { label: 'filtri incrociati (0° e 90°)', onclick: () => { st.a1 = 0; st.a2 = 90; s1.value = 0; s2.value = 90; upd(); } },
  ])));
  const out = readout('');
  ...
  function upd() {
    ...
    let msg = `<b>Legge di Malus:</b> attraverso un filtro passa cos²(differenza di angolo).\n`;
    if (!st.useThird) {
      msg += `Differenza fra i due filtri: ${Math.abs(st.a2 - st.a1)}° → cos²(${Math.abs(st.a2 - st.a1)}°) = <b>${(f.steps[1] * 100).toFixed(1)}%</b>\n`;
      if (crossed) msg += `<span class="p">Filtri incrociati (90°): non passa NIENTE.</span> Prova ad aggiungere un terzo filtro a 45° <b>in mezzo</b>…`;
    } else {
      msg += `1° → 3° (in mezzo): cos²(${Math.abs(st.a3 - st.a1)}°) = ${(f.steps[1] * 100).toFixed(1)}%\n` +
        `3° → 2°: cos²(${Math.abs(st.a2 - st.a3)}°) = ${(f.steps[2] * 100).toFixed(1)}%\n` +
        `<b>Totale: ${(f.total * 100).toFixed(1)}%</b>`;
      if (crossed && f.total > 0.01) {
        msg += `\n<span class="g">🤯 Aggiungendo un filtro la luce passa DI PIÙ, non di meno!</span>\n` +
  ...
  upd();
  w.setFoot('Un fotone polarizzato è il <b>qubit fisico</b> più semplice: verticale = |0⟩, orizzontale = |1⟩, e le direzioni oblique sono <b>sovrapposizioni</b>. La probabilità di passare è il <b>quadrato del coseno</b> dell\'angolo: ecco da dove viene "probabilità = ampiezza²".');
  return { state: st };
}
...
export function twoQubitLab(host, opts = {}) {
  ...
  const w = widget(host, { title: 'Due qubit', subtitle: 'quattro ampiezze, e la sorpresa dell\'entanglement' });
  let st = zeroState(2);
  const hist = { seq: [] };
  ...
  const hg = histogram(hgHost, { n: 2, height: 145, title: 'Risultati (q1q0)' });
  const out = readout('');
  w.body.appendChild(out.root);
  ...
  function upd() {
    ...
    out.set(
      ...
      `porte: ${hist.seq.join(' → ') || '—'}\n` +
      `test di separabilità |a·d − b·c| = <b>${det.toFixed(3)}</b> → ` +
      (ent ? '<span class="p">STATI INTRECCIATI (entangled)</span>' : '<span class="g">stato separabile: i due qubit sono indipendenti</span>') + '\n' +
      `lunghezza della freccia di Bloch: q0 = ${bv0.purity.toFixed(2)}, q1 = ${bv1.purity.toFixed(2)}` +
      (ent ? '  ← accorciate: presi da soli i qubit non hanno più uno stato definito!' : ''));
  }
  ...
  upd();
  w.setFoot('<b>Prova la ricetta di Bell:</b> premi <b>H q0</b> e poi <b>CNOT q0→q1</b>. Ottieni |00⟩ + |11⟩: quando misuri, i due qubit escono <b>sempre uguali</b>, anche se prima nessuno dei due aveva un valore. Nota che le due sfere di Bloch si accorciano fino a sparire: l\'informazione non è più nei singoli qubit, è <b>nella coppia</b>.');
...
export function teleportLab(host, opts = {}) {
  ...
  const w = widget(host, { title: 'Teletrasporto quantistico', subtitle: 'sposta uno stato senza spostare la particella' });
  ...
  const build = () => {
    ...
    s.re[0] = Math.cos(t / 2);
    s.re[1] = Math.sin(t / 2) * Math.cos(p);
    s.im[1] = Math.sin(t / 2) * Math.sin(p);
    return s;
  };
  ...
  const left = h('div'), right = h('div');
  grid.append(left, right); w.body.appendChild(grid);
  left.appendChild(h('div', { class: 'small dim', html: '<b>Qubit di partenza (Alice, q0)</b>' }));
  const sphA = blochSphere(left, { height: 210, showAngles: false });
  right.appendChild(h('div', { class: 'small dim', html: '<b>Qubit di Bob (q2)</b>' }));
  ...
  const sT = slider({ label: 'θ dello stato da mandare', min: 0, max: 180, step: 5, value: st.theta, fmt: v => v + '°', oninput: v => { st.theta = v; reset(); } });
  const sP = slider({ label: 'φ dello stato da mandare', min: 0, max: 360, step: 10, value: st.phi, fmt: v => v + '°', oninput: v => { st.phi = v; reset(); } });
  w.body.appendChild(controls(sT.root, sP.root));
  w.body.appendChild(h('div', { class: 'btn-row', style: { marginTop: '10px' } },
    h('button', { class: 'btn sm primary', onclick: () => next() }, '▶ passo successivo'),
  ...
  function upd() {
    ...
    out.set(
      `<b>Passo ${st.step + 1}/${steps.length}</b> — ${steps[st.step]}\n` +
      (st.bits[0] !== null ? `bit classici inviati ad Alice→Bob: <b>${st.bits[0]}${st.bits[1]}</b> ` +
        `(→ correzione: ${st.bits[1] ? 'X ' : ''}${st.bits[0] ? 'Z' : ''}${!st.bits[0] && !st.bits[1] ? 'nessuna' : ''})\n` : '') +
      `differenza fra lo stato di partenza e quello di Bob: <b>${dist.toFixed(4)}</b>` +
      (done ? '  <span class="g">✓ TELETRASPORTO RIUSCITO</span>' : ''));
    if (done && !st.celebrated) { st.celebrated = true; fx.win(); sfx.boss(); }
    if (done) cfg.onDone && cfg.onDone();
  }
  reset();
  w.setFoot('<b>Attenzione a cosa NON è:</b> nessuna materia si sposta e nessuna informazione viaggia più veloce della luce — servono i due <b>bit classici</b> telefonati a Bob, che viaggiano normalmente. E lo stato di partenza viene <b>distrutto</b> dalla misura: è un trasferimento, non una fotocopia. Il teorema di <b>no-cloning</b> resta salvo.');
  return { state: st };
}
