...
describe('mappa dei livelli', () => {
  test('37 livelli in 6 parti, tutti con file e XP', () => {
    assert.equal(LEVELS.length, 37);
  ...
  test('gli id sono unici e i file pure', () => {
    assert.equal(new Set(LEVELS.map(l => l.id)).size, 37);
    assert.equal(new Set(LEVELS.map(l => l.file)).size, 37);
  });
