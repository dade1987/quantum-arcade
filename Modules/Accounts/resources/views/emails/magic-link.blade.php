      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
        ...
        <tr><td>
          ...
          <h1 style="margin:0 0 18px;font-size:24px;color:#e8edf9">
            {{ $isNew ? __('Benvenuto!') : __('Bentornato!') }}
          </h1>

          <p style="margin:0 0 14px;font-size:15px;line-height:1.6;color:#9dabc9">
            @if($isNew)
              {!! __('Hai creato un account su Quantum Arcade. Clicca il bottone qui sotto per <b style="color:#e8edf9">confermare la tua email</b> ed entrare: non serve nessuna password.') !!}
            @else
              {{ __('Ecco il tuo link di accesso. Nessuna password da ricordare: clicca ed entri.') }}
            @endif
          </p>
          ...
          <p style="margin:24px 0">
            <a href="{{ $url }}"
              ...
              {{ $isNew ? __('Conferma ed entra') : __('Entra in Quantum Arcade') }}
            </a>
          </p>

          <p style="margin:0 0 14px;font-size:13px;line-height:1.6;color:#6c7a9c">
            {!! __('Il link vale <b>20 minuti</b> e si può usare <b>una sola volta</b>.') !!}<br>
            {{ __('Se il bottone non funziona, copia questo indirizzo nel browser:') }}<br>
          ...
          <p style="margin:0;font-size:12px;line-height:1.6;color:#6c7a9c">
            {{ __('Se non hai richiesto tu questo accesso, ignora questa email: senza il click non succede nulla e il tuo indirizzo non verrà usato per altro.') }}
          </p>
        </td></tr>
      </table>
      <p style="margin:18px 0 0;font-size:11px;color:#6c7a9c">Quantum Arcade · {{ __('di Davide Cavallini') }}</p>
    </td></tr>
  </table>
</body>
</html>
